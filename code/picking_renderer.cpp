...
bool init_picking_D3D(HWND window, PICKING_D3D_RESOURCES *directx_picking) {
    ...
    HRESULT device_and_swap_chain_hr = D3D11CreateDeviceAndSwapChain(0, D3D_DRIVER_TYPE_HARDWARE,
                                                      ...
                                                      &directx_picking->swap_chain, &directx_picking->device,
                                                      &directx_picking->feature_level, &directx_picking->immediate_context);
    ...
    HRESULT create_view_hr = directx_picking->device->CreateRenderTargetView(directx_picking->picking_texture, 0, &directx_picking->render_target);
    ...
    HRESULT create_vs_hr = directx_picking->device->CreateVertexShader(vs_blob->GetBufferPointer(), vs_blob->GetBufferSize(), 0, &directx_picking->vertex_shader);
    HRESULT create_ps_hr = directx_picking->device->CreatePixelShader(ps_blob->GetBufferPointer(), ps_blob->GetBufferSize(), 0, &directx_picking->pixel_shader);
    ...
    directx_picking->immediate_context->VSSetShader(directx_picking->vertex_shader, 0, 0);
    directx_picking->immediate_context->PSSetShader(directx_picking->pixel_shader, 0, 0);

    D3D11_INPUT_ELEMENT_DESC layout_desc[] = {
        {"TEXCOORD", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0},
    };
    ID3D11InputLayout *input_layout;
    HRESULT create_layout_hr = directx_picking->device->CreateInputLayout(layout_desc, ARRAYSIZE(layout_desc), vs_blob->GetBufferPointer(),
                                                                  vs_blob->GetBufferSize(), &input_layout);
    if(FAILED(create_layout_hr)) {
        LOG_ERROR("ERROR", "Cannot create input layout");
        return false;
    }
    directx_picking->immediate_context->IASetInputLayout(input_layout);
        
    return true;
}

void clean_picking_D3D(PICKING_D3D_RESOURCES *directx_picking) {
    directx_picking->device->Release();
    directx_picking->immediate_context->Release();
    directx_picking->swap_chain->Release();
    directx_picking->picking_texture->Release();
    directx_picking->render_target->Release();
    directx_picking->vertex_shader->Release();
    directx_picking->pixel_shader->Release();
    directx_picking->vertex_buffer->Release();
}
